...
async function checkContractStatus(
  ...
  let roleVerifications:
    | { [role: string]: { expectedAddress: string; hasRole: boolean; status: '✅' | '❌' } }
    | undefined;
...
function displayStatus(result: StatusResult) {
  ...
  for (const [address, hasRole] of Object.entries(result.adminRoles)) {
    const icon = hasRole ? '✅' : '❌';
    const label =
      address === ADMIN_ADDRESSES.testnet
        ? 'Test Deployer'
        : address === ADMIN_ADDRESSES.mainnet
        ? 'Main Deployer'
        : address === ADMIN_ADDRESSES.multisig
        ? 'Multisig'
        : 'Unknown';
...
task('status', 'Check status of deployed contracts and their roles')
  ...
  .setAction(async (taskArgs) => {
    ...
    try {
      ...
      const adminRolesCount = results.reduce(
        (sum, r) => sum + Object.values(r.adminRoles).filter((hasRole) => hasRole).length,
        0
      );
      const correctRolesCount = results.reduce(
        (sum, r) => sum + Object.values(r.expectedRoles).filter((role) => role.status === '✅').length,
        0
      );
      const totalExpectedRoles = results.reduce((sum, r) => sum + Object.keys(r.expectedRoles).length, 0);
