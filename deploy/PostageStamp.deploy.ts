...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  if (priceOracle) {
    ...
    if (!hasPriceOracleRole) {
      await execute('PostageStamp', { from: deployer, log: true }, 'grantRole', PRICE_ORACLE_ROLE, priceOracle);
  ...
  if (redistributor) {
    ...
    if (!hasRedistributorRole) {
      await execute('PostageStamp', { from: deployer, log: true }, 'grantRole', REDISTRIBUTOR_ROLE, redistributor);
  ...
  if (pauser) {
    ...
    if (!hasPauserRole) {
      await execute('PostageStamp', { from: deployer, log: true }, 'grantRole', PAUSER_ROLE, pauser);
