...
async function main() {
  ...
  // Get old contract
  const oldPostageStamp = (await ethers.getContractAt(
    'PostageStampLegacy',
    OLD_POSTAGE_STAMP_ADDRESS
  )) as PostageStampLegacy;
 ...
 */
async function waitForTokenTransfer(token: any, targetAddress: string, expectedAmount: any): Promise<void> {
  let attempts = 0;
